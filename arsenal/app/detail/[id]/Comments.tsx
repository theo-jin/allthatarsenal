...
export default function Comment({result}) {
  ...
  return (
    ...
    <Grid xs={12}  justify="center">
        <Text h2 css={{margin:"0px"}}>Comment</Text>
        </Grid> 
     
        {data.length>0 ? data.map(function(a,i){
          return(
            <Grid xs={12}  justify="center">
               <Card  variant="bordered" borderWeight="normal" css={{  p: "$4", mw: "500px",borderRadius:"5px" ,margin:"1px",padding:"1px"}} >
               <Grid.Container justify="center">
                <Grid xs={6}>#{i+1}. {data[i].comment} </Grid><Grid justify="flex-end" xs={6}><Modal comment={data[i].comment}/></Grid>  </Grid.Container></Card>
